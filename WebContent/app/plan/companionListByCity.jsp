			...
			<div class="inner">
				...
				<section>
					...
					<section id="search" class="alt">
						<form method="post"
							action="${pageContext.request.contextPath}/app/plan/AcomListByCity.acom">
							<input type="text" name="query" id="query"
								placeholder="여행하고자 하는  도시를 검색해 주세요" />
						</form>
					</section>
					<br>
							...
							<c:when test="${list != null and fn:length(list) > 0}">
								<c:forEach var="acom_dataMap" items="${list}">
									<article>
										...
										<h5>${acom_dataMap.get('schedule_user_id')}</h5>
										<ul class="actions">
											<li><a
												href="${pageContext.request.contextPath}/app/plan/(action수정))seq=${acom_dataMap.get('schedule_num')}"
				...
				<ul class="pagination">
					<li><c:choose>
							<c:when test="${nowPage > 1}">
								<span
									onclick="location.href='${pageContext.request.contextPath}/app/plan/AcomListByCity.acom?page=${nowPage-1}'"
					...
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<c:choose>
							...
							<c:otherwise>
								<li><a
									href="${pageContext.request.contextPath}/app/plan/AcomListByCity.acom?page=${i}"
									class="page">${i}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
